...
import com.richersoon.laboratory.integration.dto.CreateSymptomRequestDto;
...
public class VirusSymptomResource {
     ...
     */
    @PostMapping
    public SymptomDto create(@Valid @RequestBody CreateSymptomRequestDto createRequestDto,
                             @PathVariable String name) {
        SymptomRequestDto requestDto = SymptomRequestDto.builder()
                .virusName(name)
                .description(createRequestDto.getDescription())
                .build();
        return symptomService.create(requestDto);
    }
